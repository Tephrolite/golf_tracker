...
const runDatabaseTests = process.env.RUN_DATABASE_INTEGRATION_TESTS === 'true' && databaseUrl !== undefined;
...
describe.runIf(runDatabaseTests)('database security integration', () => {
  ...
  it('enables RLS and denies anon/authenticated table privileges', async () => {
    const result = await database!.unsafe<{ tablename: string; rowsecurity: boolean; anon_access: boolean; authenticated_access: boolean }[]>(`
    ...
    `);
    expect(result).toHaveLength(10); expect(result.every((table) => table.rowsecurity && !table.anon_access && !table.authenticated_access)).toBe(true);
  });
});