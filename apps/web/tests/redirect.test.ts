...
const signedOutClient: AuthClient = {
...
describe('routing security', () => {
  ...
  it('redirects unauthenticated protected navigation and preserves its internal path', async () => {
    await useAuthStore().initialize(signedOutClient);
    const router = createAppRouter(createMemoryHistory());
    await router.push('/app/rounds');
    await router.isReady();
    expect(router.currentRoute.value.path).toBe('/sign-in');
    expect(router.currentRoute.value.query.redirect).toBe('/app/rounds');
  });
  it('redirects unprovisioned authenticated users from sign-in to profile completion', async () => {
    const store = useAuthStore();
    await store.initialize(signedOutClient);
